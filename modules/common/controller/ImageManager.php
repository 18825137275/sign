...
class ImageManager extends Controller{
	...
	protected function init($dir){			
		...
		$this->user_image_dir=$dir;
		
		$this->upload_url=ROOT_PATH . 'uploads/images/'.$this->user_image_dir;
	...
	public function upload(){
		...
		if (!$json) {
			if (!empty($files['file']['name']) && is_file($files['file']['tmp_name'])) {
				...
				if (!in_array(mb_strtolower(utf8_substr(strrchr($filename, '.'), 1)), $allowed)) {
					$json['error'] = '文件类型错误';
				}
	...
	public function get_file_data($data){
		...
		foreach ($images as $image) {
			...
			if (is_dir($image)) {
				...
				$images_and_directory['images'][] = array(
					'thumb' => resize(utf8_substr($image, utf8_strlen(DIR_IMAGE)), 100, 100),
					'name'  => implode(' ', $name),
					'type'  => 'image',
					'path'  => utf8_substr($image, utf8_strlen(DIR_IMAGE)),
				);
	...
	//用于ckeditor图片上传
	function ckupload(){
		$dir = ROOT_PATH . 'uploads/images/ckeditor/'.date('Ymd',time());
		if (!is_dir($dir)) {			
			mkdir($dir, 0777);
			chmod($dir, 0777);
		...
		if (!empty($files['upload']['name']) && is_file($files['upload']['tmp_name'])) {
			...
			$ext=mb_strtolower(utf8_substr(strrchr($filename, '.'), 1));
		...
		if (!$json) {
			...
			//源文件保存路径
		 	$savepath=$this->request->domain().'/uploads/images/ckeditor/'.date('Ymd',time()).'/'.$filename;
			//保存源文件
			move_uploaded_file($files['upload']['tmp_name'], $dir . '/' . $filename);
			//生成缩略图
			$thumb=$this->request->domain().'/'.resize($thumb_path,'630');
			//删除原图
			@unlink(DIR_IMAGE.'images/ckeditor/'.date('Ymd',time()).'/'.$filename);
			
       		//下面的输出，会自动的将上传成功的文件路径，返回给编辑器。
        	echo "<script type=\"text/javascript\">window.parent.CKEDITOR.tools.callFunction(".$n.",'$thumb','');</script>";
		}else{
			echo "<script type=\"text/javascript\">window.parent.CKEDITOR.tools.callFunction(".$_GET['CKEditorFuncNum'].", '/', '上传失败," . $json['error'] . "！');</script>";
		}
